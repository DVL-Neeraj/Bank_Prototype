...
categories = {
    ...
    "employer": {"Unemployed": -0.75, "Employed": 0.1, "Self-Employed": 0.25},
    "loan_history": {
        "None": 0,
    ...
    "event": {"None": 0},
    "rm_skillset": {
        "Knowledgeable Regarding Saving Accounts": 0.1,
        "Low Knowledge": -0.1,
        "Low Effort": -0.1,
...
def assign_label_saving_account(base_df):
    ...
    df["probability"] = conversion_duration_probability(
        df[["probability", "conversion_duration"]], 35
    )
    ...
    df["label"] = df["label"].map({True: "Saving Account", False: "None"})
