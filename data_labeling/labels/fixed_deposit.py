...
categories = {
    ...
    "region": {"Urban": 0.2, "Rural": 0.3, "Negative Zone": -0.5},
    "employer": {"Unemployed": -0.5, "Employed": 0.15, "Self-Employed": 0.05},
    "event": {"None": 0},
    "rm_skillset": {
        "Knowledgeable Regarding Fixed Deposits": 0.1,
        "Low Knowledge": -0.1,
        "Low Effort": -0.1,
    ...
    "marital_status": {"Married": 0, "Unmarried": 0.0},
}


def assign_label_fixed_deposit(base_df):
    ...
    df["probability"] = df["probability"] + (df["interest_rate"] * 0.05)
    ...
    df["label"] = df["label"].map({True: "Fixed Deposit", False: "None"})
